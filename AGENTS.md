...
- TranslateService (`src/features/translate/translateService.js`) is a singleton owning its own `SttService` with `enabledSpeakers: ['Them']`. **Two stages, two providers, two keys:** STT on Deepgram `nova-3`, translation on OpenRouter `google/gemini-2.5-flash`. Each stage reads its own key from `modelStateService.getAllApiKeys()` and, when it is missing, shows a status naming *that* provider (`No Deepgram key. Open Settings → API Keys → Deepgram.`, `No Soniox key. Open Settings → API Keys → Soniox.`).
- **Translate Engine is picked in Settings** (not env; the STT Model list is Listen's): `deepgram+llm` (default), `soniox+llm`, `soniox-native` (no LLM), `openrouter+llm` — `TRANSLATE_ENGINES`, stored as `translateEngine` in `settingsService`, read at every session start. The header shows `EN → UK · <label>`. Evidence, rationale and the A/B procedure: `docs/TRANSLATE_SONIOX.md`. `initializeSession` compares the STT message by exact equality — change the constant and the comparison together. No local fallback. It does **not** use the Settings-side active STT provider/model. **Mutually exclusive** with Listen — both are in `SIDE_FEATURE_WINDOW_NAMES`, and entering one auto-stops the other via `stopForModeSwitch` / `stopListenForModeSwitch`.
...
- **One fragment per commit**, stable `segment.id` from first draft to freeze. An entry with `isFinal: true` is never updated again, so the renderer's upsert-by-id needs no notion of commits. If the commit normalizes equal to what the draft translated, `handleCommit` freezes that translation instead of spending a request. A commit never reuses an `isFinalizing` segment (it would overwrite the previous clause); Stop waits for every segment in `streamingSegments`.
- **STT provider comes from the Translate Engine.** Deepgram is a websocket with `interim_results` + `smart_format` at 24 kHz mono linear16 (~1-1.5 s to first text, `is_final` every 2-4 s **mid-sentence**); OpenRouter is chunked `/chat/completions` with `input_audio` (~6 s) and stays as the A/B baseline — don't delete it, and don't add `openrouter` to `sttModels` in `factory.js`. Whisper is no longer on this path.
- **Soniox (`providers/soniox.js`).** Key only in the first (config) frame — never log it. **Always `translation: one_way uk`, even in `soniox+llm`** (only then are finals clause units, not late mid-word windows). `sttService.handleSonioxMessage()` is the one parse point: drops `<end>`/`<fin>` **by text**, commits `translation_status === 'original'` finals. Keepalive in the provider (408 after ~20 s idle closes with code 1000); graceful `close()` via `finalize`; no auto-renewal. Not in `sttModels`.
- **Native engine lives in `translateService`** (`handleNativeUpdate`). One fragment = final original run + the translation run after it; strict O→T is not assumed (`pendingUnits`, orphan fragment, `PICKLE_TRANSLATE_NATIVE_TIMEOUT_MS`); counters logged on Stop as `[TranslateAB-native]`.
- **Translate does not register `onTranscriptionComplete`** — the debounce flush would re-deliver already-committed text. The trailing interim on Stop is committed by `closeSession()` *after* `await closeSessions()` (Soniox flushes there; equivalent for Deepgram/OpenRouter, logged as `Committing trailing draft on stop`) and *before* the abort. Never move it after the abort — that drops the last words.
...
- **Keep the `translate:translation-update` contract** `{ id, sourceText, translation, isStreaming, isFinal }` and its upsert-by-id: the Soniox native engine lands on it without renderer changes.
- **A/B logs:** `[TranslateAB] …` per frozen fragment, `[TranslateAB-session] …` once per Stop, in speech order — compare engines on the session line.
...
- Tunables (all optional; the engine is not one of them): `PICKLE_TRANSLATE_NATIVE_TIMEOUT_MS` (5000, native only), `PICKLE_TRANSLATE_LLM_MODEL` (`google/gemini-2.5-flash`), `PICKLE_TRANSLATE_STT_CHUNK_SECONDS` (5, OpenRouter only), `PICKLE_TRANSLATE_MIN_PARTIAL_GROWTH_CHARS` (30), `PICKLE_TRANSLATE_DEBOUNCE_MS` (2000), `PICKLE_TRANSLATE_TEMPERATURE` (0.1), `PICKLE_TRANSLATE_MAX_TOKENS` (1024), `PICKLE_TRANSLATE_SEGMENT_GAP_MS` (4000), `PICKLE_TRANSLATE_MIN_PARTIAL_CHARS` (4), `PICKLE_TRANSLATE_CONTEXT_TURNS` (4; 0 disables).
...
- Use `node src/features/translate/translateService.selfcheck.js` after Soniox / Translate native changes (plain Node, no network).
