...
- **STT provider is env-selected, LLM provider is not.** Translate **does not** read `modelStateService.getCurrentModelInfo()` for either stage. `PICKLE_TRANSLATE_STT_PROVIDER` picks `deepgram` (default) or `openrouter`; an unknown value silently falls back to `deepgram`. `TRANSLATE_LLM_PROVIDER` stays pinned to `openrouter` with `PICKLE_TRANSLATE_LLM_MODEL || 'google/gemini-2.5-flash'`. Settings-side LLM/STT choice affects Listen/Ask but not Translate. Each stage looks up its own key in `modelStateService.getAllApiKeys()` and names the provider that is missing: `No Deepgram key. Open Settings → API Keys → Deepgram.` for STT, `No OpenRouter key…` for translation. `initializeSession` matches the STT string by **exact equality** to decide what to show — change the constant and the comparison together, or the status degrades to a generic failure. The `flash-lite` tier was tried first for translation and produced noticeably weaker Ukrainian — don't downgrade the default without re-testing on live audio.
...
- **A dead STT socket surfaces in the UI.** A provider websocket can die mid-session (network blip, sleep, provider-side close). `sttService.handleSessionClosed(speaker, generation)` nulls the session pointer and pushes `Transcription disconnected. Press Stop, then start again.` through `onStatusUpdate`. The `generation` counter (bumped in `initializeSttSessions`) stops the 2 s overlap socket of an auto-renewal being mistaken for the live one; the `!this[key]` check keeps a deliberate `closeSessions()` quiet. There is no auto-reconnect — the user restarts the session. Known gap: if the 20-minute auto-renewal's handshake fails, `initializeSttSessions` has already nulled the pointer and the old socket is never closed, so the session stays dead and leaks a socket.
...
- **The `translate:translation-update` contract is a provider-agnostic seam.** `{ id, sourceText, translation, isStreaming, isFinal }`, upserted by `id` in the renderer. Keep the shape and the upsert logic as they are: a future move to Soniox (original + translation from one websocket, token-level finality) must land on this same contract without touching the renderer.
...
- `PICKLE_TRANSLATE_STT_PROVIDER` — `deepgram` (default) or `openrouter`. Read once at module load, so switching means restarting the app; `dotenv.config()` runs before `translateService` is required, so putting it in `.env` works.
- `PICKLE_TRANSLATE_LLM_MODEL` — OpenRouter model id for translation (default `google/gemini-2.5-flash`). Use it to A/B models on live audio without a code change.
- `PICKLE_TRANSLATE_STT_MODEL` — STT model id; defaults to `nova-3` for Deepgram and `google/gemini-2.5-flash` for OpenRouter (an OpenRouter model here must accept audio input).
